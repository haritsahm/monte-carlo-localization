...
void MCL::saveConfig()
{
    ...
    out << YAML::Key << "mcl_vis_var" << YAML::Value << mcl_vis_var;
    out << YAML::Key << "mcl_afast" << YAML::Value << mcl_afast;
    out << YAML::Key << "mcl_aslow" << YAML::Value << mcl_aslow;
    out << YAML::Key << "mcl_cmps_var" << YAML::Value << mcl_cmps_var;
...
double MCL::compass_err(double angle)
{
    if(angle>=360)
        angle -= 360;
    if(angle < 0)
    ...
    double diff = angle - robot_pos.z;

    return exp(fabs(diff)/2*mcl_cmps_var*mcl_cmps_var);
}
...
void MCL::lowVarResampling()
{
    ...
    std::cout << "particle : " << N_Particle << std::endl;
